...
*/
    ...
    protected static enum QueryRetrieveLevel {
    ...
    private static final int[] PATIENT_MATCHING_KEYS = { 
        ...
        Tag.PatientBirthDate,
        Tag.PatientSex };
//masuda^   private -> protected
    protected static final int[] STUDY_RETURN_KEYS = {
        Tag.StudyDate,
        Tag.StudyTime,
        Tag.AccessionNumber,
        Tag.StudyID,
...
//masuda^   private -> protected
    protected static final String[] IVRLE_TS = {
...
//masuda$
 ...
 //masuda^   private -> protected
    ...
    protected final List<TransferCapability> storeTransferCapability = new ArrayList<TransferCapability>(8);
//masuda$
...
*/
    ...
    public void move(List<DicomObject> findResults)
        ...
        for (int i = 0, n = Math.min(findResults.size(), cancelAfter); i < n; ++i) {
            DicomObject dokeys = findResults.get(i).subSet(MOVE_KEYS);
            if (isEvalRetrieveAET() && containsMoveDest(
                    findResults.get(i).getStrings(Tag.RetrieveAETitle))) {
                LOG.info("Skipping {}:\n{}",
                        UIDDictionary.getDictionary().prompt(cuid), dokeys);
            } else {
                LOG.info("Send Retrieve Request using {}:\n{}",
                        UIDDictionary.getDictionary().prompt(cuid), dokeys);
                assoc.cmove(cuid, priority, dokeys, tsuid, moveDest, rspHandler);
            }
        }
...
/*
    public void get(List<DicomObject> findResults)
        ...
        for (int i = 0, n = Math.min(findResults.size(), cancelAfter); i < n; ++i) {
            ...
            LOG.info("Send Retrieve Request using {}:\n{}",
                    UIDDictionary.getDictionary().prompt(cuid), dokeys);
            assoc.cget(cuid, priority, dokeys, tsuid, rspHandler);
        }
        assoc.waitForDimseRSP();
    }
*/
